...
- `export { name as default, Person }` -> `import { default as fooName, Person } from './module.js'` or `import { Person, default as fooName } from './module.js'`-> 成员重命名为`default`，表示这个成员是默认导出的，也就是说你可以直接通过`import xxx from './module.js'`拿到这个这个`name`的值 -> 注意，默认导出的成员只能有一个，也就是说一个模块里边不能出现两次`as default`，也不能是一次`as default`，一次`export default name`
- `export default name` -> `import xxx from './module.js'` -> 导出默认成员时，建议使用这种，而`export {}`这种就不要再出现`as default`这种使用了
